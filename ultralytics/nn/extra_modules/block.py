...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'C3_ContextGuided', 'C2f_ContextGuided', 'C3_MSBlock', 'C2f_MSBlock', 'ContextGuidedBlock_Down', 'C3_DLKA', 'C2f_DLKA', 'CSPStage', 'SPDConv', 'SPDConv_add',
           'BiFusion', 'RepBlock', 'C3_EMBC', 'C2f_EMBC', 'SPPF_LSKA', 'C3_DAttention', 'C2f_DAttention', 'C3_Parc', 'C2f_Parc', 'C3_DWR', 'C2f_DWR', 'LCT']
...
class SPDConv_add(nn.Module):
    # Changing the dimension of the Tensor
    def __init__(self, inc, ouc, dimension=1):
        super().__init__()
        self.d = dimension
        self.conv = Conv(inc * 4, ouc, k=3)
        self.conv2 = Conv(inc, ouc, k=3)
        # self.m = nn.MaxPool2d(kernel_size=k//2+1, stride=1)

    def forward(self, x):
        x_s = torch.cat([x[..., ::2, ::2], x[..., 1::2, ::2], x[..., ::2, 1::2], x[..., 1::2, 1::2]], 1)
        x_s = self.conv(x_s)
        m = nn.MaxPool2d(kernel_size=(x.shape[2]//2+1, x.shape[3]//2+1), stride=1)
        x_m = m(x)
        x_m = self.conv2(x_m)
        x = torch.add(x_s, x_m)
        return x
