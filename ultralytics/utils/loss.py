...
from .metrics import bbox_iou, bbox_mpdiou, wasserstein_loss, bbox_inner_iou, inner_ciou, inner_siou
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask, mpdiou_hw=None):
        ...
        # iou = bbox_inner_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True, ratio=1.0)
        # loss_iou = ((1.0 - iou) * weight).sum() / target_scores_sum
        #=====SIOU
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, SIoU=True, Focal=False)
