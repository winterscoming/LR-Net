...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, WIoU=False, scale=False, Focal=False, alpha=1, gamma=0.5, eps=1e-7):
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or WIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU or WIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                if Focal:
                    return iou - (rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2), torch.pow(inter/(union + eps), -rho2 / c2) # Focal_EIou
                else:
                ...
                angle_cost = torch.pow(torch.cos(torch.arcsin(sin_alpha) * 2 - math.pi / 2), 2)
                # rho_x = (s_cw / cw) ** 2
                # rho_y = (s_ch / ch) ** 2
                rho_z = (s_cw**2+s_ch**2) / (cw**2+ch**2)
                gamma = angle_cost - 2
                # distance_cost = 2 - torch.exp(gamma * rho_x) - torch.exp(gamma * rho_y)
                # distance_cost = 2 - torch.exp(gamma * rho_z)

                # siou收敛解决
                rho_x = (s_cw / cw) ** 2
                rho_y = (s_ch / ch) ** 2
                distance_cost = 2 - torch.exp(-(gamma+3) * rho_x) - torch.exp(-(gamma+3) * rho_y)
                # distance_cost = 2 - torch.exp((gamma-1) * rho_z)

                omiga_w = torch.abs(w1 - w2) / torch.max(w1, w2)
                omiga_h = torch.abs(h1 - h2) / torch.max(h1, h2)
                shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
                if Focal:
                    # return iou - torch.pow(0.5 * (distance_cost + shape_cost) + eps, alpha), torch.pow(inter/(union + eps), -rho_z) # Focal_SIou
                    return iou - torch.pow((distance_cost + shape_cost) * 0.5 + eps, alpha), torch.pow(inter/(union + eps), gamma) # Focal_SIou
                else:
                    return iou - torch.pow((distance_cost + shape_cost) * 0.5 + eps, alpha) # SIou
            elif WIoU:
                if Focal:
...
def wasserstein_loss(pred, target, eps=1e-7, constant=35.8):
